...
CardListPlugin.prototype.buildCardList = function(compilation, cb){
  ...
  entryCardDir(this.options.includePath, this.options.includePath, (p, v)=>{
    if(this.options.include && !p.match(this.options.include)){ return }
    if(this.options.exclude && p.match(this.options.exclude)){ return }
    const json = JSON.parse(fs.readFileSync(p, null))
    const name = _.find(json, { "@define":["name"] })
    const ext = _.find(json, { "@extend":"card/base" })
    if(ext && name){
      list.push({ value:v.replace("\\","/").split(".")[0], text:name["@define"][2] })
    }
  })
...
CardListPlugin.prototype.apply = function(compiler) {
  compiler.hooks.run.tapAsync("CardListPlugin", this.buildCardList.bind(this))
